...
uint256 hashGenesisBlock("0x4518ba2e36a2e96a5798608a774a237f1c7d3e40421fcb00a8f2da78d6cf4c94");
static CBigNum bnProofOfWorkLimit = CBigNum().SetCompact(504365644); // foocoin: starting difficulty is 1 / 2^12
...
{
    // foocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // foocoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 302400; // foocoin: 3.5 days
static const int64 nTargetSpacing = 150; // foocoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // foocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // foocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // foocoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x54a5b0a3af0c629d0586b7916df7ebcf827455561ac740076637df0e9553cc78");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "to mars with elon musk style and no less than 30 pieces of flair";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 5000000000;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("f8b94971a64b16d60c2b1877c014714baa2dab4adccda43c34bb96e9c49db0fe02b4b0fe66bce524e065e5e1277f08018b7cdfb730e0a1eaf044262588224d4d08") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515426704;
        block.nBits    = 504365644;
        block.nNonce   = 3663468843;

        if (fTestNet)
        {
            block.nTime    = 1515426704;
            block.nNonce   = 3260309648;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xe2c862e4312fac165ca64a5963136d5ba35b61aa8a23656190e787c51ce7db96"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // foocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("foocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("foocoinMiner : ProcessBlock, block not accepted");
    }
